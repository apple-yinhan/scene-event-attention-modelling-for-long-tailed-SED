...
def Focal_Multilabel_Loss(pred, label, num_labels, gamma=2):
    l = pred.reshape(-1)
    t = label.reshape(-1)
    p = l
...
if __name__ == "__main__":
    import numpy as np
    # Test Focal Loss for Multi-Classification
    B, C, X1, X2 = 32, 4, 100, 200
    pred_logit = np.random.randn(B, C, X1)
    ...
    label = np.random.randint(0, C, size=(B, X1))
    ...
    # Test Focal Loss for Multi-Label Classification
    pred_logit = torch.rand((32, 200, 25))
    label = torch.rand((32, 200, 25))
    num_labels = 25
    
    gamma = 2
    l = pred_logit.reshape(-1)
    t = label.reshape(-1)
    p = torch.sigmoid(l)
    p = torch.where( t >= 0.5, p, 1-p)
    logp = - torch.log(torch.clamp(p, 1e-4, 1-1e-4))
    ...
    loss_ = Focal_Multilabel_Loss(pred_logit, label, num_labels)
